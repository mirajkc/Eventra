...
interface EventTemplate {
  title: string;
  description: string;
  category: 'WORKSHOP' | 'MEETUP' | 'CONFERENCE' | 'WEBINAR' | 'HACKATHON' | 'COMPETITION' | 'OTHER';
  tags: string[];
}
...
interface OrgTemplate {
  ...
  orgType: 'INDIVIDUAL' | 'COMPANY' | 'EDUCATIONAL' | 'COMMUNITY' | 'NON_PROFIT' | 'GOVERNMENT';
  description: string;
...
async function seed() {
  ...
  const orgTemplateMapping: { [orgId: string]: OrgTemplate } = {};
  ...
  for (const event of events) {
    ...
    for (const participant of participantsList) {
      ...
      if (participant.id === event.creatorId) continue;
