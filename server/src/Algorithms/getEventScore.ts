import type { IEvent, IEventScore, IEventTypes } from "../lib/types/event.types.js";
function getTextScore(title : string ,description : string ){
...
function getCategoryScore(category : IEventTypes){
    switch(category) {
...
function getTagsScore (tags :Array<string>) {
  const maxTags = 5; 
  const tagCount = tags.length;
  const tagScore = Math.min(tagCount / maxTags, 1);
  return tagScore;
}
function getPremiumScore ( premium : boolean ){
  if(premium){
    return 1 
  } else {
    return 0
...
function getThumbnailScore( url : string | null| undefined ){
  if(url && url.length > 1){
    return 1
  }else{
    return 0
...
export default function getEventScore(event : IEventScore) {
