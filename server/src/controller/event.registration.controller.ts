...
class EventRegistrationController {
  ...
  async registerNewParticipant(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const postJoinTasks: Array<Promise<unknown>> = [
      ...
      if (groupNotification.length > 0) {
        postJoinTasks.push(notificationService.sendManyNotification(groupNotification) as Promise<unknown>)
      }
  ...
  async removeRegistration(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const groupNotification: Array<ICreateNotificaion> =
        ...
        const postLeaveTasks: Promise<any>[] = [
          notificationService.sendNotificaion({
            userId: userDetails.id,
