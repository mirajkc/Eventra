...
class CreditController {
    ...
    async getStripeURI(req, res, next) {
        try {
            //package, 50, 100, 200, 
    ...
    async handleWebhook(req, res) {
        try {
            ...
            let event;
            try {
                event = stripe.webhooks.constructEvent(req.body, sig, enviroment.stripeWebHookSigningSecret || '');
            }
            catch (err) {
            ...
            if (event.type === 'checkout.session.completed') {
                ...
                await emailService.sendEmail({
                    ...
                    message: thanksForCreditPurchase(userDetails.name, organizationDetails.name, Number(credits))
                });
                const groupNotification = organizationDetails.members?.map((m) => ({
                    userId: m.userId,
