...
class ChatService {
    dataDir = path.resolve(process.cwd(), "src", "chatbot", "data");
    async loadKnowledgeBase(locale) {
    ...
    stripEmojis(text) {
        return text.replace(/[\p{Extended_Pictographic}\u{1F1E6}-\u{1F1FF}\u{2600}-\u{27BF}]/gu, "");
    }
    sanitizeReply(text) {
        return this.stripEmojis(text).replace(/\s{2,}/g, " ").trim();
    }
    buildUrl(frontendBaseUrl, path) {
        return new URL(path, frontendBaseUrl.endsWith("/") ? frontendBaseUrl : `${frontendBaseUrl}/`).toString();
    }
    buildLink(label, path, frontendBaseUrl) {
        return {
            label,
            url: this.buildUrl(frontendBaseUrl, path)
        };
    }
    getStaticRouteLinks(message, frontendBaseUrl) {
        const normalized = message.toLowerCase();
        const links = [];
    ...
    appendHelpfulLinks(reply, links) {
        if (links.length < 1) {
    ...
    buildDeterministicReply(message, frontendBaseUrl) {
        const normalized = message.toLowerCase();
        if (/join|join.*organization|organization.*join|how can i join|become a member/.test(normalized)) {
    ...
    buildSystemPrompt(knowledgeBase, frontendBaseUrl) {
    ...
    async generateReply({ message, locale, frontendBaseUrl }) {
